<script lang="ts">
	...
	const getData = async () => {
		const resp = await fetch(`${API_URL}/api/indicator/stoch?symbol=${COIN}&interval=${INTERVAL}`);
		const json = (await resp.json()) as DTValue<[number, number, number]>[];
		const k: LineData[] = toSingleValueDataOfIdx(json, 0);
		const d: LineData[] = toSingleValueDataOfIdx(json, 1);
		return [k, d];
	};
</script>
